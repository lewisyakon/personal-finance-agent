...
HEADER_ALIASES = {
    ...
    "platform_category": ("当前状态", "平台分类", "类别"),
}
...
class WechatCsvParser:
    ...
    def parse(self, path: Path) -> ParseReport:
        ...
        for offset, row in enumerate(data_rows, start=header_index + 2):
            if not any(_clean(cell) for cell in row):
                continue
            try:
                records.append(self._record(row, columns, offset))
    ...
    def _record(
        ...
        type_value = self._cell(row, columns["type"])
        description = self._cell(row, columns["description"])
        return TransactionRecord(
            platform=self.source,
            occurred_at=_parse_datetime(self._cell(row, columns["occurred_at"])),
            direction=_direction(self._cell(row, columns["direction"])),
            amount_minor=_parse_amount(self._cell(row, columns["amount"])),
            status=_status(self._cell(row, columns["status"])),
            merchant=self._cell(row, columns["merchant"]),
            description=description or type_value,
            payment_method=self._cell(row, columns["payment_method"]),
            platform_category=self._cell(row, columns["platform_category"]),
            source_transaction_id=self._cell(row, columns["source_transaction_id"]) or None,
